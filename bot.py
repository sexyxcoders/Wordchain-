# bot.py — ᴛɴᴄ ᴡᴏʀᴅᴄʜᴀɪɴ ᴄᴏɴᴛʀᴏʟʟᴇʀ ʙᴏᴛ
import asyncio
...
# ────────────────────────────────
# 🔒 ᴍᴜꜱᴛ-ᴊᴏɪɴ ᴠᴇʀɪꜰɪᴄᴀᴛɪᴏɴ
# ────────────────────────────────
async def check_membership(client, user_id):
    """Ensure the user has joined all required channels."""
...
async def start_cmd(client, message):
    ...
    caption = (
        ...
        "ꜱᴇɴᴅ <code>/connect STRING</code> ᴛᴏ ʀᴇɢɪꜱᴛᴇʀ ʏᴏᴜʀ ᴛᴇʟᴇᴛʜᴏɴ ꜱᴇꜱꜱɪᴏɴ.\n"
...
async def recheck_membership(client, callback_query):
    ...
    if await check_membership(client, user_id):
        await callback_query.message.edit_caption(
            "✅ ʏᴏᴜ ʜᴀᴠᴇ ᴊᴏɪɴᴇᴅ! ʏᴏᴜ ᴄᴀɴ ɴᴏᴡ ᴜꜱᴇ ᴛʜᴇ ʙᴏᴛ."
        )
    else:
...
async def connect_cmd(client, message):
    ...
    try:
        ...
        await message.reply_text(
            "🤖 ʏᴏᴜʀ ᴜꜱᴇʀʙᴏᴛ ɪꜱ ᴀᴄᴛɪᴠᴇ ᴀɴᴅ ʀᴇᴀᴅʏ ᴛᴏ ᴘʟᴀʏ!",
            parse_mode=ParseMode.HTML
        )
    except Exception as e:
        await message.reply_text(
            f"❌ ꜰᴀɪʟᴇᴅ ᴛᴏ ꜱᴛᴀʀᴛ ᴜꜱᴇʀʙᴏᴛ.\nError: {e}",
            parse_mode=ParseMode.HTML
        )
...
async def disconnect_cmd(client, message):
    ...
    if user.id == config.OWNER_ID and len(args) > 1:
        ...
        await message.reply_text(
            f"✅ ᴅɪꜱᴄᴏɴɴᴇᴄᴛᴇᴅ ᴜꜱᴇʀ <code>{target}</code>",
            parse_mode=ParseMode.HTML
        )
        await client.send_message(
            config.LOG_GROUP_ID,
            f"❌ ᴜꜱᴇʀʙᴏᴛ ᴅɪꜱᴄᴏɴɴᴇᴄᴛᴇᴅ ʙʏ ᴏᴡɴᴇʀ: {target}",
            parse_mode=ParseMode.HTML
        )
        return
    ...
    await client.send_message(
        config.LOG_GROUP_ID,
        f"🧹 ᴜꜱᴇʀ ᴅɪꜱᴄᴏɴɴᴇᴄᴛᴇᴅ: {user.id}",
        parse_mode=ParseMode.HTML
    )


# ────────────────────────────────
# 📢 /broadcast + /listusers + /stats
# ────────────────────────────────
